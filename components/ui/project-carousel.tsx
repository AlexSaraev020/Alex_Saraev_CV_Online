...
import WeVibe1 from "@/public/images/projects/weVibe/1.webp";
import WeVibe2 from "@/public/images/projects/weVibe/2.webp";
import WeVibe3 from "@/public/images/projects/weVibe/3.webp";
import WeVibe4 from "@/public/images/projects/weVibe/4.webp";
import WeVibe5 from "@/public/images/projects/weVibe/5.webp";
import WeVibe6 from "@/public/images/projects/weVibe/6.webp";
  ...
  const projects = [
    {
      ...
      title: "WeVibe",
      description:
        "A social media platform that uses modern technologies and concepts such as REST, JWT Authentification, complex NextJs Routing for a responsive and dynamic user experience and much more. WeVibe is designed to create a modern and user-friendly social media platform.",
      technologies: [
        "NextJs",
        "MongoDB",
        "Typescript",
    ...
    },
    {
      ...
      description:
        "Stay informed with the latest news from around the world with an easy-to-use app. Simply search for news stories and read them directly within the app. Whether you are interested in global events or specific topics, the platform offers a straightforward way to access and stay updated on current news.",
    ...
    <section className="w-full text-white flex">
      {currentProject ? (
        <div
          ...
          className="w-full flex flex-col lg:flex-row px-4 md:px-20 py-10"
        >
          <div className="w-full lg:w-6/12">
            <h1 className=" text-3xl md:text-6xl font-bold">
              {currentProject.title}
            </h1>
            <p className="text-sm md:text-lg md:p-4 pt-4 md:pt-0 md:w-10/12">
              {currentProject.description}
            </p>
            {currentProject.note && (
              <p className="text-sm md:text-lg pt-4 md:pt-0 md:p-4 w-10/12 font-extrabold text-emerald-500">
                {currentProject.note}
              </p>
            )}
            <ul className="w-fit gap-2 grid grid-cols-2 md:p-4 py-8 md:py-0">
              {currentProject.technologies ? (
                currentProject.technologies.map((technology, index) => (
                  <MagicCard key={index}>{technology}</MagicCard>
                ))
              ) : (
          ...
          <div className="w-full lg:w-6/12 flex flex-col gap-8 items-center justify-center">
            ...
            <div className="flex gap-8 text-2xl">
              <a
                className="border-2 bg-zinc-900 hover:shadow-glow hover:shadow-zinc-400 hover:scale-110 transition-all duration-500 z-10 border-zinc-500 rounded-xl px-4 py-2"
              ...
              <div className="overflow-x-scroll py-4 scrollbar-thin scrollbar-thumb-zinc-600 scrollbar-track-transparent">
                <div className="flex gap-4 min-w-max">
                  {projects.map((project) => (
                    <div
                      className="flex flex-col relative border-2 border-zinc-500 rounded-xl shadow-glow-sm hover:shadow-white transition-all duration-500"
                      key={project.id}
                    >
                      <Image
                        width={300}
                      ...
                      <button
                        ...
                        className="absolute inset-0 bottom-0 left-0 bg-black/10 flex items-end justify-center w-full text-white text-center"
                      >
                        {project.title}
                      </button>
                    </div>
